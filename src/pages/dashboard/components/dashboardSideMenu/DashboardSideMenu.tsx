...
export default function DashboardSideMenu({ products, party }: Props) {
      ...
      subMenu: [
        { groupId:'selfCare',
          ...
          isSelected: (location) =>  {
            ...
            console.log("match",matchPath(location.pathname, ROUTES.PARTY_DASHBOARD.path)); 
            return matchPath(location.pathname, ROUTES.PARTY_DASHBOARD.path) !== null ;},
    ...
    products
      ...
      .map((p) => ({
        ...
        subMenu: [
          ...
          canSeeRoles ? { groupId:p.id, title: 'Ruoli', active: p.authorized ?? false } : undefined,
        ],
      }))
  );
  ...
  const handleClick = (
    ...
    setSelectedItem((menuItem.groupId === selectedItem?.groupId && menuItem.subMenu )? null : menuItem);
    if (menuItem.onClick) {
      menuItem.onClick();
    }
  };
  ...
  return (
    <Grid container item mt={11}>
      <Grid item xs={12}>
        <List>{arrayMenu && arrayMenu.map(item => <DashboardSideMenuItem key={item.title} item={item} selectedItem={selectedItem} handleClick={handleClick} location={location} />)}</List>
      </Grid>
    </Grid>
  );
}
