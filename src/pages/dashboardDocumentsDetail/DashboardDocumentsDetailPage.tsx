...
const DashboardDocumentsDetail = ({ party }: DocDetailsProps) => {
      ...
      {!showBackComponent && (
        ...
        >
          <Link
            color="text.primary"
            underline="hover"
            href={resolvePathVariables(DASHBOARD_ROUTES.DOCUMENTS.path, {
          ...
          <Grid>
            {documents.map((doc, i) => (
              <Grid container key={i} alignItems="center" spacing={1}>
                {doc.contractAvailable ? <InsertDriveFileIcon color="primary" /> : <FileCopyOff />}
                <Typography variant="body1">{doc.productId}</Typography>
